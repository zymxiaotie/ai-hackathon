# modules/report_generator.py - POSTGRES VERSION
...
def generate_report(tender_id: int, conn, output_path: str = None) -> str:
    ...
    # 1. Tender master data
    cur.execute("SELECT * FROM tenders WHERE tender_id = %s", (tender_id,))
    tender = cur.fetchone()  # Returns dict because of RealDictCursor
    if not tender:
    ...
    cur.execute("""
        ...
        FROM tender_addenda WHERE tender_id = %s ORDER BY received_date
    """, (tender_id,))
    addenda = cur.fetchall()

    # 5. Recommendation logic
    met_criteria = sum(1 for c in criteria if c['is_met'] is True)
